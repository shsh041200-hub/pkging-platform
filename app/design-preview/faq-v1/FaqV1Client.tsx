...
// ─── FAQ content (no content changes — UX/UI only) ────────────────────────────
...
// ─── AnchorButton: copies URL+hash to clipboard ───────────────────────────────
...
// ─── FaqAccordion: single item with anchor + open state ──────────────────────

function FaqAccordion({ item }: { item: FaqItem }) {
  const [open, setOpen] = useState(false)

  // open via URL hash on mount
  useEffect(() => {
...
export function FaqV1Client() {
  return (
    <div className="min-h-screen" style={{ background: T.bg }}>
      {/* Header — production pattern */}
