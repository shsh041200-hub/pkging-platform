...
const FEATURES = [
  {
    ...
    icon: (
      <svg className="w-6 h-6 text-[#533afd]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.6}>
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="bg-[#f6f9fc] border-b border-[#e5edf5]">
        <div className="max-w-6xl mx-auto px-5 py-16 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div>
            <p className="text-xs font-semibold tracking-widest uppercase text-[#533afd] mb-4">포장재 B2B 디렉토리 플랫폼</p>
            <h1 className="text-[36px] sm:text-[46px] font-bold text-[#061b31] leading-[1.15] tracking-[-0.03em] mb-5">
              포장재 파트너,<br />지금 무료로<br />찾아보세요
            </h1>
            <p className="text-base text-[#64748d] leading-relaxed mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href="/"
                className="bg-[#533afd] hover:bg-[#4434d4] text-white px-7 py-3.5 rounded font-semibold text-[15px] transition-colors shadow-[rgba(83,58,253,0.2)_0px_8px_20px_-4px] text-center"
              >
                지금 업체 찾기
              </Link>
              <Link
                href="#"
                className="border border-[#b9b9f9] text-[#533afd] px-7 py-3.5 rounded font-semibold text-[15px] hover:bg-[rgba(83,58,253,0.04)] transition-colors text-center"
              >
                업체로 등록하기
              </Link>
            </div>
          ...
          <div>
            {/* Trust indicator */}
            <div className="bg-[#533afd] text-white rounded-xl p-4 mb-4 flex items-center justify-between">
              <div>
                <p className="text-[28px] font-light tracking-[-0.03em]">{VENDOR_COUNT}</p>
                <p className="text-[13px] text-[#b9b9f9] mt-0.5">등록 업체 수</p>
              </div>
              <div className="w-px h-10 bg-white/20" />
              <div>
                <p className="text-[28px] font-light tracking-[-0.03em]">12</p>
                <p className="text-[13px] text-[#b9b9f9] mt-0.5">포장 카테고리</p>
              </div>
              <div className="w-px h-10 bg-white/20" />
              <div>
                <p className="text-[28px] font-light tracking-[-0.03em]">무료</p>
                <p className="text-[13px] text-[#b9b9f9] mt-0.5">검색·비교</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.slice(0, 4).map((cat) => (
                <Link
                  key={cat.label}
                  href={cat.href}
                  className="group border border-[#e5edf5] rounded-lg p-3.5 bg-white hover:border-[#533afd]/30 transition-all flex items-center gap-2.5"
                >
                  <div className="w-8 h-8 rounded bg-[rgba(83,58,253,0.08)] flex items-center justify-center flex-shrink-0 group-hover:bg-[rgba(83,58,253,0.12)] transition-colors">
                    <div className="w-3 h-3 rounded-sm bg-[#533afd]/30" />
                  </div>
                  <div>
      ...
      <section className="max-w-6xl mx-auto px-5 py-16">
        <h2 className="text-xl font-light text-[#061b31] tracking-[-0.02em] text-center mb-10">Packlinx 이렇게 활용하세요</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="border border-[#e5edf5] rounded-xl p-6 bg-white text-center"
            >
              <div className="w-12 h-12 rounded-xl bg-[rgba(83,58,253,0.08)] mx-auto mb-4 flex items-center justify-center">
                {f.icon}
              </div>
              <h3 className="text-[16px] font-semibold text-[#061b31] mb-2">{f.title}</h3>
              <p className="text-[13px] text-[#64748d] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {CATEGORIES.map((cat) => (
              <Link
                key={cat.label}
                href={cat.href}
                className="group border border-[#e5edf5] rounded-lg px-4 py-3 bg-white hover:border-[#533afd]/30 hover:shadow-[rgba(83,58,253,0.06)_0px_4px_12px] transition-all flex items-center justify-between"
              >
                <div>
                  <p className="text-[13px] font-semibold text-[#061b31] group-hover:text-[#533afd] transition-colors">{cat.label}</p>
                  <p className="text-[11px] text-[#64748d] mt-0.5">{cat.sub}</p>
                </div>
                <span className="text-[#64748d]/40 group-hover:text-[#533afd] transition-colors text-sm">→</span>
              </Link>
            ))}
          </div>
        </div>
