...
export const metadata: Metadata = {
  title: '[PREVIEW] companies-v1 — vendor 상세 UX 개선안',
...
export default async function CompaniesV1Preview({ params }: Props) {
  ...
  // Collect all contact methods for the sticky CTA
  ...
  // Related guides for primary industry category
  const relatedGuides = primaryIndustry ? (RELATED_GUIDES[primaryIndustry] ?? []) : []

  return (
    <>
      {/* Preview banner */}
      <div className="bg-brand-700 text-white text-center py-2 px-4">
        <p className="text-[12px] font-medium tracking-wide">
          [PREVIEW] companies-v1 — PACAA-548 UX 개선안 · 비공개 · production과 별도 라우트
        </p>
      </div>

      <div className="min-h-screen bg-neutral-50">
        {/* Header — production component reuse */}
          ...
          */}
          <div className="bg-white border border-border-v04 rounded-xl overflow-hidden" style={{ boxShadow: 'var(--shadow-elevated-v04)' }}>
            ...
            <div className="p-5 sm:p-7">
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <h1 className="text-[22px] sm:text-[26px] heading-display text-heading-deep-navy tracking-[-0.02em] leading-tight mb-1.5">
                      {company.name}
                    </h1>
                ...
                {/* Right: primary CTA (desktop visible) — 모바일은 sticky bottom bar 사용 */}
                {hasAnyContact && (
              ...
              {/* Core commerce stats — scan-friendly row */}
              {hasCoreCommerce && (
              ...
              {/* Trust signal — data source + verification (moved from hero bottom to prominent inline) */}
          ...
          {/* ═══ CONTACT CARD — V1: 연락처를 별도 카드로 분리, 더 스캐너블하게 ═══ */}
          {hasAnyContact && (
              ...
              <div className="flex flex-col sm:flex-row gap-2.5">
                {hasWebsite && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 text-[14px] font-semibold text-white bg-stripe-purple hover:bg-stripe-purple-hover px-4 py-3 rounded-lg transition-colors"
                  >
                ...
                {hasPhone && (
                  <a
                    href={`tel:${company.phone}`}
                    className="flex-1 flex items-center justify-center gap-2 text-[14px] font-semibold text-stripe-purple border border-stripe-purple-ring bg-stripe-purple-soft hover:bg-stripe-purple-tint px-4 py-3 rounded-lg transition-colors"
                  >
                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                ...
                {hasEmail && (
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-2 text-[14px] font-semibold text-neutral-700 border border-neutral-200 bg-neutral-50 hover:bg-neutral-100 px-4 py-3 rounded-lg transition-colors"
                  >
                    <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* ═══ SIMILAR COMPANIES — V1: tighter cards, category count badge ═══ */}
          ...
          {/* ═══ RELATED GUIDES — V1: 신규 섹션. 구매자 다음 행동으로 가이드 노출 ═══ */}
        ...
        {/* ═══ STICKY MOBILE CTA — V1 신규: 모바일 fold 문제 해결 ═══
            ...
            웹사이트 또는 전화 중 우선순위 높은 것 하나만 노출 (단순화).
        */}
        {hasAnyContact && (
        ...
        {/* Footer — production component reuse */}
