    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="bg-neutral-50 border-b border-neutral-200 py-20 px-5">
        <div className="max-w-2xl mx-auto text-center">
          ...
          {/* Search bar */}
          <form className="flex rounded-lg overflow-hidden border border-neutral-200 shadow-md bg-white focus-within:border-brand-500 focus-within:shadow-md transition-shadow">
            <input
              type="search"
              placeholder="업체명, 제품, 소재로 검색..."
              className="flex-1 px-5 py-4 text-[15px] text-neutral-900 placeholder:text-neutral-500/60 focus:outline-none bg-white"
            />
            <button
              type="submit"
              className="bg-brand-500 hover:bg-brand-600 text-white font-medium px-6 py-4 transition-colors flex-shrink-0 text-sm"
            >
              검색하기
            </button>
          </form>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-6">
            {CATEGORIES.slice(0, 6).map((cat) => (
              <Link
                key={cat.label}
                href={cat.href}
                className="text-[13px] text-brand-500 border border-brand-200 bg-white px-3.5 py-1.5 rounded-full hover:bg-brand-500/[0.05] transition-colors"
              >
                {cat.label}
              </Link>
            ))}
          ...
          <p className="mt-6 text-xs text-neutral-500">
            현재 <span className="font-semibold text-neutral-900">{VENDOR_COUNT}</span>개 업체 등록 · 무료 이용
          </p>
        </div>
      </section>
