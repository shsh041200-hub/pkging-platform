      ...
      {!showingCategory && <section className="max-w-7xl mx-auto px-5 sm:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {industry && (
              <span className="text-[11px] bg-stripe-purple/6 text-stripe-purple font-medium px-2.5 py-1 rounded-full">
            ...
            {selectedMaterials.map((mat) => (
              <Link
                key={mat}
                href={buildMaterialUrl(mat)}
                className="text-[11px] bg-stripe-purple/6 text-stripe-purple font-medium px-2.5 py-1 rounded-full flex items-center gap-1 hover:bg-stripe-purple/12 transition-colors"
              >
                {MATERIAL_TYPE_LABELS[mat]}
                <span className="text-stripe-purple/60 text-[10px] leading-none">×</span>
              </Link>
            ))}
        ...
        {companies && companies.length > 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {companies.map((company) => (
              <article
                key={company.id}
                className="bg-white border border-border-v04 rounded-xl overflow-hidden hover:border-stripe-purple/20 hover:shadow-[var(--shadow-elevated-v04)] hover:-translate-y-px transition-all duration-200 group relative flex flex-col min-h-[260px] max-h-[280px]"
              >
                <div className="p-6 flex flex-col flex-1 overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    />
                    <h2 className="text-base font-bold text-heading-deep-navy leading-snug tracking-[-0.02em] line-clamp-1 flex-1 min-w-0" title={company.name}>
                      <Link
