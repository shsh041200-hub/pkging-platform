    ...
    <>
      ...
      <div className="-mx-5 sm:-mx-8 -mt-10 sm:-mt-14">
        ...
        >
          <div className="max-w-[1180px] mx-auto px-6">
            ...
            <h1 className="text-[44px] leading-[1.18] tracking-[-0.025em] m-0 mb-3 font-extrabold text-[var(--g-ink)]">
              발주 전에 꼭 확인하는<br className="hidden sm:block" />패키징 실무 가이드
            </h1>
            <p className="text-lg leading-[1.6] text-[var(--g-ink-2)] max-w-[720px] m-0 mb-6">
              소재 선택부터 MOQ·납기·인증까지. 1,380개 업체 데이터를 기반으로 만든 현장형 가이드 — 추측 대신 비교, 검색 대신 결정.
            </p>
            ...
            >
              ...
              <input
                ...
                className="flex-1 border-0 outline-none font-[inherit] text-base bg-transparent text-[var(--g-ink)] placeholder:text-[var(--g-ink-3)] cursor-default"
              />
              <button
                type="button"
            ...
            {/* Chip filters (UI only — Phase 3) */}
            <div className="mt-3 flex flex-wrap gap-2">
              {["전체", "처음 발주", "MOQ 비교", "친환경·ESG", "인쇄 품질", "2026 트렌드", "이사·물류"].map(
                (chip, i) => (
                  <span
                    key={chip}
                    className={`text-[13px] px-3 py-1.5 rounded-full cursor-default border transition-colors ${
                      i === 0
        ...
        <main className="max-w-[1180px] mx-auto px-6 pb-8">
          ...
          {/* Featured cards — 2fr 1fr 1fr grid */}
          <div
            className="mb-10 grid gap-[18px]"
            ...
            {FEATURED.map((feat, i) => (
              ...
              >
                <div
                  ...
                  style={{
                    aspectRatio: "16/9",
                    background: thumbBackground(feat.thumbVariant),
                  }}
                >
                  <div
                ...
                <div className={`px-[22px] pt-5 pb-[22px] flex-1 flex flex-col ${feat.isLarge ? "" : ""}`}>
                  ...
                  {i === 0 && (
                    <p className="m-0 text-[var(--g-ink-3)] text-sm leading-[1.6]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[18px]">
                {section.guides.map((g) => (
                  ...
                  >
                    ...
                    <p className="m-0 text-[var(--g-ink-3)] text-[13.5px] leading-[1.55]">
                      {g.desc}
                    </p>
                    <div className="mt-auto flex items-center justify-between text-xs text-[var(--g-ink-3)]">
                      <span>⏱ {g.time}</span>
                      <span className="text-[var(--g-brand-2)] font-semibold group-hover:underline">읽기 →</span>
                    </div>
                  </Link>
                ))}
              </div>
