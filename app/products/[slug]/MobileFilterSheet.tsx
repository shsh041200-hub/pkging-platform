...
interface ProductMobileFilterSheetProps {
  isOpen: boolean
  onClose: () => void
  resultCount: number
}
...
export function ProductMobileFilterSheet({
  isOpen,
  onClose,
  resultCount,
}: ProductMobileFilterSheetProps) {
  ...
  const handleApply = () => {
    ...
    if (pendingMaterials.size > 0) params.set('material', Array.from(pendingMaterials).join(','))
  ...
  const totalPending = pendingMaterials.size + pendingCerts.size + (pendingSample ? 1 : 0)
        ...
        <div className="overflow-y-auto flex-1">
          ...
          >
            {MATERIAL_TYPES.map((m) => (
              <label key={m} className="flex items-center gap-2.5 py-1.5 text-sm text-slate-700 cursor-pointer">
                <input
                  type="checkbox"
