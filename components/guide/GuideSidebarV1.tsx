...
// V1 change #3: CTA button and hover link uses --color-brand-500 (purple) instead of --g-brand (navy)
export function GuideSidebarV1({
  ctaHeadline,
  ctaSubtext,
  ctaButtonLabel,
  ctaHref,
  ...
  return (
    <aside className="hidden lg:flex flex-col gap-[18px] sticky top-[120px] self-start text-sm">
      {/* Vendor CTA — V1: brand gradient keeps dark (trust), CTA button → purple */}
      <div
      ...
      <div className="bg-white border border-[var(--g-line)] rounded-[14px] p-5">
        ...
        <div className="flex gap-2">
          {[
            { label: "🔗 링크", fn: copyLink },
            { label: "📧 메일", fn: shareEmail },
            { label: "💬 카톡", fn: shareKakao },
          ].map(({ label, fn }) => (
            <button
              key={label}
              onClick={fn}
      ...
      {/* Related guides — V1 #3: hover → purple */}
            ...
            {relatedGuides.map((g) => (
              <li key={g.href} className="py-[10px] border-b border-[#f1f3f5] last:border-0">
                <Link
                  ...
                  style={{ ["--hover-color" as string]: "var(--color-brand-500)" }}
                  onMouseEnter={(e) =>
