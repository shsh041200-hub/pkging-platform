...
export function CompanyDetailCTA({ companyId, website, kakaoUrl, phone, email }: Props) {
  ...
  const handleEmailClick = () => {
    track('email_click', companyId, { email })
  }

  const hasContact = !!phone || !!email
  const bothContacts = !!phone && !!email

  return (
    <div className="flex flex-col gap-3">
      {hasContact && (
        <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
          <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3">연락처</p>
          <div className={`grid ${bothContacts ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'} gap-3`}>
            {phone && (
              <div>
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">전화</p>
                <a
                  href={`tel:${phone}`}
            ...
            {email && (
              <div>
                <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">이메일</p>
                <a
                  href={`mailto:${email}`}
